...
const SONGS_DIR = path.join(__dirname, 'storage', 'images', 'users');
...
app.get('/song/:name', (req, res) => {
  ...
  fs.access(filePath, fs.constants.F_OK, (err) => {
      if (err) {
          return res.status(404).json({ message: "Song not found for this name" });
      }
...
// app.listen(PORT, () => {
//   console.log(`🎵 Birthday Song API is running at http://localhost:${PORT}`);
// });
